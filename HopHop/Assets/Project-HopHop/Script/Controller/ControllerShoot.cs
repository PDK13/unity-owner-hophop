...
public class ControllerShoot : MonoBehaviour
{
    ...
    private void SetControlAction()
    {
        if (m_turnTime == 0)
        {
            m_turnCommand = m_dataAction.ActionList[m_dataAction.Index];
            m_turnTime = m_dataAction.TimeList[m_dataAction.Index];
            m_turnTimeCurrent = 0;
        }
        //
        m_turnTimeCurrent++;
        ...
        if (TurnEnd)
        {
            ...
            if (m_dataAction.TypeList == DataBlockType.Forward && m_dataAction.Index > m_dataAction.DataCount - 1)
            {
                //End Here!!
            }
            else
            if (m_dataAction.TypeList == DataBlockType.Loop && m_dataAction.Index > m_dataAction.DataCount - 1)
            {
                m_dataAction.Index = 0;
            }
            else
            if (m_dataAction.TypeList == DataBlockType.Revert && (m_dataAction.Index < 0 || m_dataAction.Index > m_dataAction.DataCount - 1))
            {
